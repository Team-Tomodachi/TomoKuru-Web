...
const getEventMessages = async (eventID) => {
  const url = `${API_HOST_URL}/api/events/messages/${eventID}`;
  logRequest(getEventMessages.name, url, null);
  let response = await axios.get(url);
  logResponse(getEventMessages.name, response);
  return response;
};

const postEventMessage = async (messageObject) => {
  const requestBody = {
    user_id: messageObject.user_id,
    message: messageObject.message,
    event_id: messageObject.event_id,
    photo_url: messageObject.photo_url || "",
  };
  const url = `${API_HOST_URL}/api/events/messages/${messageObject.event_id}`;
  logRequest(postEventMessage.name, url, requestBody);
  let response = await axios.post(url, requestBody);
  logResponse(postEventMessage.name, response);
};
